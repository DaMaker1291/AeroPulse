...
interface AccountCreationProps {
  onComplete: (userData: { name: string; org: string; role: string }) => void;
}
...
export function AccountCreation({ onComplete }: AccountCreationProps) {
  ...
  const handleComplete = () => {
    setCompleting(true);
    setTimeout(() => {
      onComplete({ name: form.fullName || 'Dr. Sarah Chen', org: form.organization || 'Memorial Hospital', role: form.role || 'Neurologist' });
    }, 1800);
  };
  ...
  return (
    <div className="w-screen h-screen bg-[#0B0B0D] flex overflow-hidden">
      ...
      {/* Right: Form Panel */}
      <div className="flex-1 flex flex-col items-center justify-center p-12 overflow-auto bg-[#0D0D10]">
        {!completing ? (
