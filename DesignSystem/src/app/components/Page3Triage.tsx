  ...
  const metricCards = [
    {
      ...
      value: `${metrics.bilateralSymmetry}%`,
      sub: 'Within normal baseline variance',
      icon: Activity,
      status: 'ok',
      bar: metrics.bilateralSymmetry,
    },
    {
      label: 'Neuromuscular Lag',
      value: `${metrics.neuromuscularLag} ms`,
      sub: 'Optimal path latency',
      icon: TrendingUp,
      status: 'ok',
      bar: 100 - (metrics.neuromuscularLag / 200) * 100,
    },
    {
      label: 'Vascular Compliance',
      value: metrics.vascularCompliance,
      sub: 'Cross-referenced age profile',
      icon: AlertTriangle,
      status: 'warn',
      bar: 55,
    },
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="bg-[#16161A] rounded-2xl border border-[#1E1E22] p-5 flex-shrink-0">
        ...
        <div className="grid grid-cols-3 gap-4">
          {metricCards.map(card => {
            ...
            const accentColor = isWarn ? '#FF9F0A' : '#30D158';
            ...
            return (
              <div key={card.label} className="bg-[#0B0B0D] rounded-xl border border-[#1E1E22] p-4">
                ...
                <div className="text-[22px] font-bold leading-tight mb-1" style={{ color: isWarn ? accentColor : 'white' }}>
                  {card.value}
                </div>
      ...
      <div className="flex-1 bg-[#16161A] rounded-2xl border border-[#1E1E22] p-5 flex flex-col min-h-0">
        ...
        <div className="flex-1 bg-[#0B0B0D] rounded-xl border border-[#1E1E22] p-4">
